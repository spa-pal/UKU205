 	...
 	if((UIB2[1]==0x51)&&(UIB2[4]==1))
		...
		if(memo_out2[16]!=0xff)
			...
			for(i=0;i<memo_out2[5];i++)
				...
				memo_out2[lengt++]=(char)t_b; 		//Температура батареи (дискретность ?С, со знаком), младший байт.
				memo_out2[lengt++]=(char)(t_b/256); //Температура батареи (дискретность ?С, со знаком), старший байт.
